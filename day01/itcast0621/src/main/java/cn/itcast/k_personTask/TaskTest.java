package cn.itcast.k_personTask;
...
public class TaskTest {
	...
	public void deployementProcessDefinitionAndStartProcessInstance(){
		...
		//启动流程实例的同时，设置流程变量，对应task.bpmn中办理人设置的#{userID}
		Map<String, Object> variables = new HashMap<String, Object>();
		variables.put("userID", "洪七公");
		String processDefinitionKey = "task";
		ProcessInstance pi = processEngine.getRuntimeService()//管理执行对象和流程实例相关的Service（正在执行）
					.startProcessInstanceByKey(processDefinitionKey,variables);//使用流程定义的key启动流程实例，因为当使用流程定义key的时候，默认启动的是最新版本
