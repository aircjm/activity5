...
public class sequenceFlowTest {
	...
	public void deployementProcessDefinitionAndStartProcessInstance(){
		...
		Deployment deployment = processEngine.getRepositoryService()//与流程定义和部署对象相关的Service
					...
					.name("连线")//指定部署名称
					.addInputStream("sequenceFlow.bpmn", inBpmn)//使用inputStream格式的输入流完成部署
					.addInputStream("sequenceFlow.png", inPng)//使用inputStream格式的输入流完成部署
					.deploy();//完成部署
...
//						.singleResult()//惟一结果集
	...
	public void completeTask(){
		 ...
		 * 赋值给sequenceFlow.bpmn中连线的条件：${message=='重要'}
